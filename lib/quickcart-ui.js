...
(function (root, factory) {
  ...
  DEFAULT_TEMPLATES.item = [
    '<tr ' + DATA_ATTRIBUTE_PREFIX_ITEM + 'id="{{id}}">',
      '<td>',
        '{{properties.name}}',
        '<br>',
        '<button ' + DATA_ATTRIBUTE_PREFIX + 'action="remove" ' + DATA_ATTRIBUTE_PREFIX_ITEM + 'id="{{id}}">Remove</button>',
      '</td>',
      '<td>',
        '{{localizedPrice}}',
      '</td>',
      '<td>',
        '<input type="number" value="{{quantity}}" ' + DATA_ATTRIBUTE_PREFIX + 'action="update" ' + DATA_ATTRIBUTE_PREFIX_ITEM + 'id="{{id}}">',
      '</td>',
      '<td>',
        '{{localizedSubtotal}}',
      '</td>',
  ...
  QuickCartView.prototype.add = function(item) {
    ...
    var rendered = this.templates.item.render(this.getContext(item));
  ...
  function QuickCart(doc, options) {
    ...
    this.templates = this.options.templates || {
      item: Hogan.compile(DEFAULT_TEMPLATES.item), // TODO: remove hogan dependency by pre-compiling templates
    };
    this.doc = doc;
    this.view = null;
  ...
  QuickCart.browserStorage = function(type) {
    var engine = window[type + 'Storage']
    ...
    return {
      save: function(data) {
        engine.setItem(storageKey, JSON.stringify(data));
      },
      load: function() {
        var saved = engine.getItem(storageKey);
